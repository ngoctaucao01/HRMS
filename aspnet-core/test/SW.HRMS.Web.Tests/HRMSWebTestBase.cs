...
{
    public abstract class HRMSWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static HRMSWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(HRMSWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<HRMSDbContext> action)
        {
            using (var context = IocManager.Resolve<HRMSDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<HRMSDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<HRMSDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<HRMSDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<HRMSDbContext, Task<T>> func)
        {
            T result;
